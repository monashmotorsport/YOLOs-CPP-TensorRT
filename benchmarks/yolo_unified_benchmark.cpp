...
public:
    DetectionWrapper(const std::string& model_path, const std::string& labels_path)
        : detector_(std::make_unique<det::YOLODetector>(model_path, labels_path)) {}
...
public:
    SegmentationWrapper(const std::string& model_path, const std::string& labels_path)
        : detector_(std::make_unique<seg::YOLOSegDetector>(model_path, labels_path)) {}
...
public:
    PoseWrapper(const std::string& model_path, const std::string& labels_path)
        : detector_(std::make_unique<pose::YOLOPoseDetector>(model_path, labels_path)) {}
...
public:
    OBBWrapper(const std::string& model_path, const std::string& labels_path)
        : detector_(std::make_unique<obb::YOLOOBBDetector>(model_path, labels_path)) {}
...
public:
    ClassificationWrapper(const std::string& model_path, const std::string& labels_path)
        : classifier_(std::make_unique<cls::YOLOClassifier>(model_path, labels_path)) {}
    
    std::string getDevice() const override { return "gpu"; }  // TensorRT is always GPU
...
std::unique_ptr<UnifiedDetector> createDetector(const BenchmarkConfig& config) {
    if (config.task_type == "detection" || config.task_type == "det") {
        return std::make_unique<DetectionWrapper>(config.model_path, config.labels_path);
    } else if (config.task_type == "segmentation" || config.task_type == "seg") {
        return std::make_unique<SegmentationWrapper>(config.model_path, config.labels_path);
    } else if (config.task_type == "pose") {
        return std::make_unique<PoseWrapper>(config.model_path, config.labels_path);
    } else if (config.task_type == "obb") {
        return std::make_unique<OBBWrapper>(config.model_path, config.labels_path);
    } else if (config.task_type == "classification" || config.task_type == "cls") {
        return std::make_unique<ClassificationWrapper>(config.model_path, config.labels_path);
    }
...
void printUsage(const char* prog) {
    ...
    std::cout << "  (TensorRT always uses GPU — no --gpu/--cpu flag needed)\n";
    ...
    std::cout << "  " << prog << " quick models/yolo11n.trt data/dog.jpg\n";
    std::cout << "  " << prog << " image yolo11 det models/yolo11n.trt models/coco.names data/dog.jpg\n";
    std::cout << "  " << prog << " video yolo11 det models/yolo11n.trt models/coco.names data/video.mp4 --iterations=500\n";
}
...
BenchmarkConfig parseArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--gpu" || arg == "gpu" || arg == "--cpu" || arg == "cpu") {
            // TensorRT always uses GPU — ignore these flags for backward compat
        }
...
int main(int argc, char** argv) {
    try {
        ...
        if (mode == "quick") {
            if (argc < 4) {
                std::cerr << "Usage: " << argv[0] << " quick <model_path> <image_path>\n";
                return 1;
            }
